...
def parse_args():
    parser = argparse.ArgumentParser(description="Linear evaluation on frozen encoders")
    ...
    parser.add_argument("--lr-head", type=float, default=1e-3, help="Learning rate for linear head")
    parser.add_argument("--weight-decay", type=float, default=1e-4, help="Weight decay for AdamW")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--experiment-name", type=str, default="linear_eval", help="Experiment name")
...
def main():
    ...
    log_path = results_dir / "linear_eval_log.csv"
    ...
    optimizer = torch.optim.AdamW(classifier.parameters(), lr=args.lr_head, weight_decay=args.weight_decay)
    ...
    torch.save(classifier.state_dict(), ckpt_dir / "linear_classifier.pt")
    save_metrics(results_dir / "linear_eval_metrics.json", {"test_acc": test_acc, "test_loss": test_loss})
    print(f"Finished linear evaluation. Test accuracy: {test_acc:.3f}")
