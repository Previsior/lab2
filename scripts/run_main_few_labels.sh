...
SIMCLR_EXP="${DATASET}_${BACKBONE}_simclr_few"
...
LINEAR_EXP="${DATASET}_${BACKBONE}_simclr_linear_few"
...
FT_EXP="${DATASET}_${BACKBONE}_simclr_finetune_few"
python finetune.py \
    --dataset "${DATASET}" --data-dir ./data --backbone "${BACKBONE}" \
    --encoder-type resnet --encoder-checkpoint "${SIMCLR_ENCODER}" --projection-checkpoint "${SIMCLR_PROJ}" \
    --proj-mode head_on_hfeat \
...
AE_EXP="${DATASET}_${BACKBONE}_ae_few"
...
AE_FT_EXP="${DATASET}_${BACKBONE}_ae_finetune_few"
python finetune.py \
    --dataset "${DATASET}" --data-dir ./data --backbone "${BACKBONE}" \
